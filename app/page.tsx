...
export default function HomePage() {
  ...
  const [toolsOpen, setToolsOpen] = useState(false) // mobile: fold secondary controls
  const [legendOpen, setLegendOpen] = useState(false) // mobile: collapse the legend
  ...
  return (
    <div className="relative h-full w-full overflow-hidden">
      ...
      {/* ── Top-left: brand + search + controls ── */}
      <div className="absolute top-3 left-3 z-10 w-[330px] max-w-[calc(100vw-24px)] max-h-[calc(100dvh-1.5rem)] overflow-y-auto">
        <div className="bg-stone-900/95 backdrop-blur rounded-2xl shadow-2xl border border-stone-700/50 overflow-hidden">
          <div className="px-3.5 py-2.5 flex items-center gap-2.5 border-b border-stone-800">
            <Icon name="tent" className="w-5 h-5 text-green-500 shrink-0" />
            <div className="min-w-0">
              <h1 className="text-sm font-bold text-white leading-tight">Colorado Camp Finder</h1>
              <p className="hidden sm:block text-[10px] text-stone-500">{campgrounds.length} campgrounds · rec.gov + CO State Parks</p>
            </div>
          </div>
          ...
          {/* Mobile: fold secondary tools away so the map stays visible */}
          <button
            type="button" onClick={() => setToolsOpen((o) => !o)}
            className="sm:hidden w-full flex items-center justify-center gap-1.5 text-[11px] font-semibold text-stone-400 hover:text-stone-200 border-t border-stone-800 py-2"
          >
            <Icon name={toolsOpen ? 'chevronUp' : 'chevronDown'} className="w-3.5 h-3.5" />
            {toolsOpen ? 'Fewer options' : 'More options'}
          </button>

          <div className={`${toolsOpen ? 'block' : 'hidden'} sm:block px-3 pt-1 pb-3`}>
            <button
      ...
      {/* ── Legend (collapses to a "Key" chip on mobile) ── */}
      <div className={`${selected ? 'hidden sm:flex' : 'flex'} flex-col items-end gap-1.5 absolute bottom-8 right-3 z-10`}>
        <div className={`${legendOpen ? 'block' : 'hidden'} sm:block bg-stone-900/90 backdrop-blur rounded-xl border border-stone-700/50 p-2.5 sm:p-3 shadow-xl`}>
          ...
          <div className="flex items-center gap-2 mt-1 pt-1.5 border-t border-stone-700/50">
            <span className="w-3 h-3 rounded-full border border-white/40" style={{ background: '#8b5cf6' }} />
            <span className="text-[11px] text-stone-300">CO State Park</span>
          </div>
        </div>
        {/* Mobile-only toggle chip */}
        <button
          type="button" onClick={() => setLegendOpen((o) => !o)}
          className="sm:hidden flex items-center gap-1.5 bg-stone-900/90 backdrop-blur rounded-full border border-stone-700/50 px-3 py-1.5 shadow-lg"
        >
          <span className="flex -space-x-1">
            {['#22c55e', '#ef4444', '#3b82f6'].map((c) => (
              <span key={c} className="w-3 h-3 rounded-full border border-stone-900" style={{ background: c }} />
            ))}
          </span>
          <span className="text-[11px] font-semibold text-stone-300">Key</span>
          <Icon name={legendOpen ? 'chevronDown' : 'chevronUp'} className="w-3 h-3 text-stone-500" />
        </button>
      </div>
      ...
      <div className="p-4">
        ...
        <div className="mt-1.5 flex items-center gap-2 flex-wrap">
          <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[11px] font-semibold text-white"
            style={{ background: isCpw ? '#8b5cf6' : STATUS_COLORS[status] }}>
            {isCpw ? 'CO State Park' : `${STATUS_LABEL[status]}${status !== 'first-come' && status !== 'unknown' ? ` · ${c.availableSites}/${c.totalSites}` : ''}`}
          </span>
          {c.campgroundType && !isCpw && <span className="text-[11px] text-stone-400">{c.campgroundType}</span>}
        </div>
        ...
        <div className="mt-3 flex gap-2">
          <a href={c.reserveUrl} target="_blank" rel="noopener noreferrer"
            className={`flex-1 text-center text-white text-sm font-semibold rounded-lg py-2 transition-colors ${isCpw ? 'bg-violet-600 hover:bg-violet-500' : 'bg-green-600 hover:bg-green-500'}`}>
            {isCpw ? 'Reserve on cpwshop →' : 'Go to site →'}
          </a>
